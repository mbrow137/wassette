...
#[allow(dead_code)]
pub async fn build_html_to_markdown_component() -> Result<PathBuf> {
    let top_level =
        PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").context("CARGO_MANIFEST_DIR not set")?);

    let component_path = top_level
        .join("examples/html-to-markdown-rs/target/wasm32-wasip2/release/html_to_markdown_rs.wasm");

    let status = tokio::process::Command::new("cargo")
        .current_dir(top_level.join("examples/html-to-markdown-rs"))
        .args(["build", "--release", "--target", "wasm32-wasip2"])
        .status()
        .await
        .context("Failed to execute cargo component build")?;
    ...
    if !status.success() {
        anyhow::bail!("Failed to compile html-to-markdown-rs component");
    }
